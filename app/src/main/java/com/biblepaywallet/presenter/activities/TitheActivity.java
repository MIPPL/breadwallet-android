...
import com.biblepaywallet.presenter.activities.settings.WebViewActivity;
import com.biblepaywallet.presenter.activities.util.BRActivity;
import com.biblepaywallet.presenter.customviews.BRButton;
import com.biblepaywallet.presenter.customviews.BRNotificationBar;
import com.biblepaywallet.presenter.customviews.BRSearchBar;
import com.biblepaywallet.presenter.customviews.BRText;
import com.biblepaywallet.presenter.customviews.BRTitheSearchBar;
...
import com.biblepaywallet.tools.manager.FontManager;
import com.biblepaywallet.tools.manager.InternetManager;
import com.biblepaywallet.tools.manager.SyncManager;
import com.biblepaywallet.tools.manager.TitheManager;
import com.biblepaywallet.tools.manager.TxManager;
...
import com.biblepaywallet.wallet.wallets.util.CryptoUriParser;
...
import static com.biblepaywallet.tools.animation.BRAnimator.t1Size;
import static com.biblepaywallet.tools.animation.BRAnimator.t2Size;
...
public class TitheActivity extends BRActivity implements InternetManager.ConnectionReceiverListener, OnTxListModified, SyncManager.OnProgressUpdate {
    private static final String TAG = TitheActivity.class.getName();
    ...
    private BRTitheSearchBar searchBar;
    ...
    private static TitheActivity app;

    private InternetManager mConnectionReceiver;

    private TestLogger logger;

    public static TitheActivity getApp() {
        return app;
    }
...
//                CryptoUriParser.processRequest(WalletActivity.this, "bitcoin:?r=https://bitpay.com/i/HUsFqTFirmVtgE4PhLzcRx", wm);
                BRAnimator.showSendFragment(TitheActivity.this, null);
        ...
        mReceiveButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                BRAnimator.showReceiveFragment(TitheActivity.this, true);
...
//        Log.d(TAG, "Current wallet ISO -> " + wm.getIso(this));
        ...
        TitheManager.getInstance().init(this);
    ...
    private void updateUi() {
        ...
        if (wallet.getUiConfiguration().buyVisible) {
            ...
            mBuyButton.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    Intent intent = new Intent(TitheActivity.this, WebViewActivity.class);
                    intent.putExtra("url", HTTPServer.URL_BUY);
                    Activity app = TitheActivity.this;
...
//        String fiatIso = BRSharedPrefs.getPreferredFiatIso(this);
        ...
        TitheManager.getInstance().updateTitheList(TitheActivity.this);
...
//
    ...
    protected void onResume() {
        ...
        BRExecutor.getInstance().forLightWeightBackgroundTasks().execute(new Runnable() {
            ...
            public void run() {
                if (wallet.getPeerManager().getConnectStatus() != BRCorePeer.ConnectStatus.Connected)
                    wallet.connectWallet(TitheActivity.this);
            }
        });
        ...
        wallet.addSyncListeners(new SyncListener() {
            ...
            public void syncStarted() {
                SyncManager.getInstance().startSyncing(TitheActivity.this, wallet, TitheActivity.this);
            }
        });
    ...
    public void onConnectionChanged(boolean isConnected) {
        ...
        if (isConnected) {
            ...
            final BaseWalletManager wm = WalletsMaster.getInstance(TitheActivity.this).getCurrentWallet(TitheActivity.this);
            BRExecutor.getInstance().forLightWeightBackgroundTasks().execute(new Runnable() {
                @Override
                public void run() {
                    final double progress = wm.getPeerManager()
                            .getSyncProgress(BRSharedPrefs.getStartHeight(TitheActivity.this,
                                    BRSharedPrefs.getCurrentWalletIso(TitheActivity.this)));
//                    Log.e(TAG, "run: " + progress);
                    if (progress < 1 && progress > 0) {
                        SyncManager.getInstance().startSyncing(TitheActivity.this, wm, TitheActivity.this);
                    }
                }
            });
    ...
    class TestLogger extends Thread {
        ...
        public void run() {
            ...
            while (true) {
                ...
                for (BaseWalletManager w : WalletsMaster.getInstance(TitheActivity.this).getAllWallets()) {
                    builder.append("   " + w.getIso(TitheActivity.this));
                    ...
                    double progress = w.getPeerManager().getSyncProgress(BRSharedPrefs.getStartHeight(TitheActivity.this, w.getIso(TitheActivity.this)));
